...
function createAccountingForInventoryTransaction(flatObject) {
  ...
  if (isArray) {
    for (let i = 0; i < lineData.length; i++) {
      ...
      let invTransactionId = flatLineObject["invTransactionId"];
      if (invTransactionId) {
        getJournalProfileAndCreateAccounting(flatObject,invTransactionId)
      }
    }
  } else {
    let flatLineObject = {};
...
function getJournalProfileAndCreateAccounting(flatObject,invTransactionId) {
  ...
  let sql =
    "SELECT gpl.* from " +
    " inoerp.gl_journal_profile_l gpl " +
    " JOIN inoerp.inv_transaction_type itp ON itp.journal_profile_header_id = gpl.gl_journal_profile_h_id ";
  " AND itp.inv_transaction_code='" + invTransactionCode + "'; ";
  ...
  let journalAccountProfiles = getAccountProfileLines(flatObject);
  let journalAccounts = journalAccountProfiles["data"];
  let flatJournalAccounts = [];
  for (let i = 0; i < journalAccounts.length; i++) {
    const line = journalAccounts[i];
    let flatLineObject = {};
    getFlatObject(line, flatLineObject);
    flatJournalAccounts.push(flatLineObject);
  }
  ...
  if (isArray) {
    for (let i = 0; i < journalProfileLines.length; i++) {
      const line = journalProfileLines[i];
      let flatJournalProfileLine = {};
      getFlatObject(line, flatJournalProfileLine);
      ...
      for (let index = 0; index < flatJournalAccounts.length; index++) {
        ...
        if(flatJournalAccountLine == glAcLineType){
            let sql =   "  INSERT INTO gl_journal_line (line_num, reference_entity_name, reference_key_name, reference_key_value, status, ledger_id," +
            ...
            "', 'U', ledger_id," +
            "   '" +
            flatJournalProfileLine["glAcProfileAcId"] +
            "' , costed_amount,costed_amount,'"+ drOrCr+"',ac_currency,ac_currency" +
  ...
  consoleLog("\n\n getJournalProfile insertSql length " + insertSql.length.toString() + " insertSql " + insertSql);
...
function getAccountProfileLines(flatObject) {
  let sql = "";
  if (
    flatLineObject["glAcProfileHeaderId"] &&
    flatLineObject["glAcProfileHeaderId"].length > 0
  ) {
    ...
    let invTransactionCode = flatObject["invTransactionCode"];
    sql =
      "call GetAllAccounts('"+invTransactionCode+"','','','','ITEM','','','','','SUBINVENTORY','INVENTORY','BUSINESS_UNIT','LEGAL_ORGANIZATION')";
  }
...
function xxgetAccountId(flatLineObject) {
  let sql =
    " SELECT * " +
    " FROM inoerp.gl_accounting_rule " +
    " WHERE 1 = 1 " +
