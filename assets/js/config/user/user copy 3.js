...
function restRequest(inputData, type, newRequest) {
  ...
  LOG("restRequest 000000000000000000001 "+ type)
  printNestedObject(newRequest);
  
  let response;
  switch (type) {
     case restRequestType.post:
      response = post(newRequest);
      break;
    case restRequestType.patch:
      response = patch(newRequest);
      break;
    case restRequestType.delete:
      response = remove(newRequest);
      break;
    case restRequestType.get:
    default:
      response = get(newRequest);
      break;
  }
  ...
  if (Array.isArray(body)) {
    for (let i = 0; i < body.length; i++) {
      let e = body[i];
      LOG("patchRequest response row number : " + i);
      printNestedObject(e);
    }
  } else {
...
function deleteRequest(inputData) {
  ...
  let newRequest = {
    host: "http://localhost:8085/api/ierp/Subinventory(subinventoryId=45)",
    content: content,
  };
...
function postRequest(inputData) {
  let content = {
    "subinventory": "TestSub02",
    "description": "TestSub02 desc",
    "type": "RAW",
...
function sqlInsertTest(inputData) {
  ...
  sql +=
    " VALUES ('TEST_VG02', 'TEST Value Group02', 'user', 'NONE', '1', '1' ) ";
  request = {
    sql: sql,
    dbType: "MySQL",
    connName: "password",
