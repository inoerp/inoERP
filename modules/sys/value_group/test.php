...
//	$sql = " SHOW COLUMNS FROM path ";
//	$prepare = $dbc->connection->prepare($sql);
//	$prepare->execute();
//	$result_fetchAll = $prepare->fetchAll(PDO::FETCH_GROUP);
//	 echo '<pre>';
//	print_r($result_fetchAll);
//	
...
//}
//$query1= "ALTER TABLE sys_value_group_line 
//	ADD `allow_budgeting_cb` boolean AFTER account_qualifier,
//	ADD `allow_posting_cb` boolean AFTER allow_budgeting_cb ";
//
////$query1= "ALTER TABLE sys_value_group_header 
////  CHANGE ` sys_value_group_header_id` `sys_value_group_header_id` int(12) unsigned NOT NULL AUTO_INCREMENT ";
////
...
//////$result = $db->query($query);
global $db;
//$query = "SHOW COLUMNS FROM sys_value_group_line ";
////$result = $db->find_by_sql($query);
//$prepare = $dbc->connection->prepare($query);
//$prepare->execute();
//$result = $prepare->fetchAll(PDO::FETCH_COLUMN, 0);
echo '<pre>';
print_r(dbObject::show_column_from_table('sys_value_group_line'));
