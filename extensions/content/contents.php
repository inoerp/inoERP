...
if ($childs_of_parent_id_array && (count($childs_of_parent_id_array) > 0)) {
 ...
 foreach ($childs_of_parent_id_array as $child_content) {
	$childs_of_parent_id .= '<li><a href="contents.php?content_type=' . $content_type_name . '&content_id=' . $child_content->content_id . '"   
...
if (!empty($content->parent_id)) {
 ...
 $parent_of_content = '<a href="contents.php?content_type=' . $content_type_name . '&content_id=' . $parent_content->content_id . '"   
...
if (!empty($category)) {
 ...
 foreach ($category as $object) {
	$category_statement .= '<a href=' .
					"\"contents.php?content_type=$content_type_name&category_id=$object->category_id\">" .
					$object->category . '</a> &nbsp; &nbsp; ';
 }
}
...
if ((!empty($content->content_id)) && ($content_type_name)) {
 require_once('contents_template.php');
}  elseif((!empty($category_id)) && ($content_type_name)){
  require_once('contents_list_template.php');
}
else {
 include_once 'content_search.php';
}
