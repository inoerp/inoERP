...
//get Price Details
function getPriceDetails(rowClass, item_id_m, price_date, price_list_headerId, json_url) {
 json_url = (typeof json_url !== 'undefined') ? json_url : 'modules/mdm/price_list/json_price_list.php';
 price_list_headerId = (typeof price_list_headerId !== 'undefined') ? price_list_headerId : '1';
 $.ajax({
	url: json_url,
	type: 'get',
	dataType: 'json',
	data: {
	 price_list_header_id: price_list_headerId,
	 find_price: 1,
	...
	success: function(result) {
	 if (result) {
		...
		$('#content').find(rowClass).find('.unit_price').val('');
	 }
...
//enable lines with change data for save
 $('#content').on('change', 'input, select', function() {
	...
	if ($('#form_line').find(trClass).length > 0) {
...
// var coa_id = $('#coa_id').val();
 ...
 itemNumber.extra_elements = ['item_id', 'item_id_m', 'item_description', 'uom_id', 'processing_lt'];
...
//remove row
 ...
 //default date
// $('#content').find('.default_date').each(function(){
...
//context menu
 if ($('ul#js_contextMenu_data').length > 0) {
	...
	classContextMenu.btn2DivId = (typeof btn2DivId !== 'undefined') ? btn2DivId : null;
