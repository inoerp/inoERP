 ...
 $.ajax({
  ...
  success: function(result) {
   if (result) {
    ...
    $('#live_display_data').append(divHtml);
   }
  },
  complete: function() {
...
// }
 function drag_drop_condition_value() {
  $(".draggable_element").draggable(
   {helper: 'clone'},
  {cursor: "crosshair"});
  $(".condition_row_value, .sort_fields_field_value,.condition_row_parameter").droppable({
   accept: ".draggable_element",
   drop: function(event, ui) {
     $(this).append($(ui.draggable).clone());
//		remove_from_dragged_element();
...
//	});
  ...
  $(".condition_row").each(function() {
   ...
   if (operator_type == 'database')
   {
    ...
    var value = $(this).find('.condition_row_value_input').val();
    var whereClause = parameter + " " + operator + " " + '\'' + value + '\'';
   }
