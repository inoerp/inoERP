...
//mandatory and field sequence
 ...
 mandatoryCheck.header_id = 'prj_percent_header_id';
...
//setting the first line & shipment number
 ...
 if ($('#bu_org_id').val() && (!$('#prj_percent_header_id').val()) && ($('#bu_org_id').attr('disabled') != 'disabled')) {
  getBUDetails($('#bu_org_id').val());
 }

 $('#transaction_type').on('change', function () {
  if ($(this).val() == 'PO_DEFAULT') {
   var link = 'select.php?class_name=po_all_v&po_status=%3DAPPROVED&mode=9&action_class_name=prj_percent_header';
...
//get supplier details
 ...
 $(".prj_percent_header_id.select_popup").on("click", function () {
  void window.open('select.php?class_name=prj_percent_header', '_blank',
          'width=1200,height=1000,TOOLBAR=no,MENUBAR=no,SCROLLBARS=yes,RESIZABLE=yes,LOCATION=no,DIRECTORIES=no,STATUS=no');
 });
...
//remove po lines
 $("#remove_row").click(function () {
  $('input[name="prj_percent_line_id_cb"]:checked').each(function () {
...
//set the line quantity & price
 ...
 if ($('#bu_org_id').val() && (!$('#prj_percent_header_id').val())) {
...
function match_purchase_order() {
 var prj_percent_header_id = $("#prj_percent_header_id").val();
 var supplier_site_id = $("#supplier_site_id").val();
 var doc_currency = $("#doc_currency").val();
 if (prj_percent_header_id && supplier_site_id) {
  var link = 'multi_select.php?window_type=popup&class_name=ap_po_matching_v&action=match_purchase_order&mode=9&action_class_name=prj_percent_line&po_status=%3DAPPROVED'
  link += '&prj_percent_header_id=' + prj_percent_header_id + '&supplier_site_id=' + supplier_site_id+ '&inv_doc_currency=' + doc_currency;
...
function apply_transaction() {
 ...
 var supplier_site_id = $("#supplier_site_id").val();
 if (prj_percent_header_id && supplier_site_id) {
  var link = 'multi_select.php?window_type=popup&class_name=prj_percent_all_v&action=apply_transaction&mode=9&action_class_name=prj_percent_cm&transaction_type=%3DINVOICE&account_type=ITEM&po_status=%3DAPPROVED'
  link += '&cm_transaction_header_id=' + prj_percent_header_id + '&supplier_site_id=%3D' + supplier_site_id;
  var po_header_id = $("#po_header_id").val();
  var po_number = $("#po_number").val();
  if (po_header_id) {
   link += '&po_header_id=' + po_header_id;
...
function create_accounting() {
 var prj_percent_header_id = $("#prj_percent_header_id").val();
 if (prj_percent_header_id) {
  var link = 'multi_select.php?window_type=popup&class_name=prj_percent_header&action=create_accounting&mode=9&action_class_name=prj_percent_header&prj_percent_header_id=' + prj_percent_header_id;
  localStorage.removeItem("reset_link");
  localStorage.setItem("reset_link", link);
  localStorage.removeItem("jsfile");
  localStorage.setItem("jsfile", "modules/ap/prj_percent/extra_prj_percent.js");
  void window.open(link, '_blank',
          'width=1200,height=1000,TOOLBAR=no,MENUBAR=no,SCROLLBARS=yes,RESIZABLE=yes,LOCATION=no,DIRECTORIES=no,STATUS=no');
  return false;
 } else {
