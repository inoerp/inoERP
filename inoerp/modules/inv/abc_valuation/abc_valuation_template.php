 ...
 <form action=""  method="post" id="inv_abc_valuation"  name="inv_abc_valuation"><span class="heading">ABC Valuation </span>
  <div class="large_shadow_box tabContainer">
   <ul class="column header_field"> 
    ...
    <li><label>Criteria</label><?php echo $f->select_field_from_object('criteria', inv_abc_valuation::abc_criteria(), 'option_line_code', 'option_line_value', $$class->criteria, 'criteria', '', 1, $readonly); ?></li>
    <li><label>Cost Type</label><?php echo $f->select_field_from_object('cost_type', bom_cost_type::find_all(), 'cost_type_code', 'cost_type', $$class->cost_type, 'cost_type', '', '', $readonly); ?></li>
    <li><label>MRP Plan</label><?php echo $f->select_field_from_object('fp_mrp_header_id', fp_mrp_header::find_all(), 'fp_mrp_header_id', 'mrp_name', $$class->fp_mrp_header_id, 'fp_mrp_header_id', '', '', $readonly, '', '', '', 'org_id'); ?></li>
    <li><label>Forecast</label><?php echo $f->select_field_from_object('fp_forecast_header_id', fp_forecast_header::find_all(), 'fp_forecast_header_id', 'forecast', $$class->fp_forecast_header_id, 'fp_forecast_header_id', '', '', $readonly, '', '', '', 'org_id'); ?></li>
   </ul>
  </div>
