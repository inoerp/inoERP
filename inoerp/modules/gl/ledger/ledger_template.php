      ...
      <div class="large_shadow_box"> 
       <ul class="column header_field">
        <li> <label><img src="<?php echo HOME_URL; ?>themes/images/serach.png" class="gl_ledger_id select_popup clickable">
          Ledger Id</label><?php echo $f->text_field_dsr('gl_ledger_id'); ?>
         <a name="show" href="form.php?class_name=gl_ledger&<?php echo "mode=$mode"; ?>" class="show document_id gl_ledger_id"><img src="<?php echo HOME_URL; ?>themes/images/refresh.png"/></a> 
        </li>
        <li><label>Ledger(1)</label><?php echo $f->text_field_dm('ledger'); ?></li>
        <li><label>Description</label><?php echo $f->text_field_dm('description'); ?></li>
        <li><label>Char of Account</label><?php echo $f->select_field_from_object('coa_id', coa::find_all(), 'coa_id', 'description', $$class->coa_id, 'coa_id', '', 1, $readonly1, '', '', '', 'coa_structure_id'); ?></li>
        <li><label>Account Structure</label><?php echo $f->select_field_from_object('coa_structure_id', coa::coa_structures(), 'option_header_id', 'option_type', $$class->coa_structure_id, 'coa_structure_id', '', 1, 1); ?></li>
        <li><label>Currency</label><?php echo $f->select_field_from_object('currency_code', option_header::currencies(), 'option_line_code', 'option_line_code', $$class->currency_code, 'currency_code', '', 1, $readonly1); ?></li>
        <li><label>Calendar</label><?php echo $f->select_field_from_object('calendar_option_line_code', gl_calendar::gl_calendar_names(), 'option_line_code', 'option_line_value', $$class->calendar_option_line_code, 'option_line_code', '', 1, $readonly1); ?></li>
        <li><label>Future Periods</label><?php form::number_field_d('future_enabled_periods'); ?></li>
