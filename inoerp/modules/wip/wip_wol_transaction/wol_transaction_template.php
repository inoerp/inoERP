<div id="wip_wol_transaction_divId">
 ...
 <div id ="form_header"> <span class="heading">Work Order Less Transaction </span> 
  <div id="form_serach_header"><ul class="inline_list">
    <li><label>WOL Id : </label>
     <?php echo $f->text_field_dsr('wip_wol_transaction_id'); ?>
    </li>
...
//        pa($wip_wo_bom_object);
        foreach ($wip_wo_bom_object as $wip_wo_bom) {
         ...
         <tr class="wip_wol_transaction0" id="tab1_1">
          <td>    
           <ul class="inline_action">
            ...
            <li><input type="checkbox" name="line_id_cb" value="<?php echo!empty($$class_second->bom_sequence) ? $$class_second->bom_sequence : '' ?>">
             ...
             <?php echo form::hidden_field('bom_header_id', $$class_second->bom_header_id); ?></li>     
           </ul>
          </td>
          <td><?php echo!empty($bom_sequence_stament) ? $bom_sequence_stament : form::text_field_wids('bom_sequence'); ?></td>
          <td><?php $f->text_field_wid2sr('bom_line_id'); ?></td>
          ...
          <td><?php echo $f->select_field_from_object('component_uom', uom::find_all(), 'uom_id', 'uom_name', $item_i->uom_id, '', 'uom_id', '', $readonly); ?></td>
          <td><?php echo $f->text_field('quantity', ''); ?></td>
         </tr>
         <?php
         $count = $count + 1;
        }
      ...
      <table class = "form_line_data_table">
       ...
       <tbody class = "inv_transaction_values form_data_line_tbody">
        <tr class = "wip_wol_transaction0" id = "tab2_1">
         <td>
          <?php echo form::select_field_from_object('from_subinventory_id', subinventory::find_all_of_org_id($$class->org_id), 'subinventory_id', 'subinventory', $$class_second->from_subinventory_id, '', $readonly, 'subinventory_id');
          ?>
         </td>
         <td>
          <?php echo form::select_field_from_object('from_locator_id', locator::find_all_of_subinventory($$class_second->from_subinventory_id), 'locator_id', 'locator', $$class_second->from_locator_id, '', $readonly, 'subinventory_id'); ?>
         </td>
         <td>
          <?php echo form::select_field_from_object('to_subinventory_id', subinventory::find_all_of_org_id($$class->org_id), 'subinventory_id', 'subinventory', $$class_second->to_subinventory_id, '', $readonly, 'subinventory_id'); ?>
         </td>
         <td>
          <?php echo form::select_field_from_object('to_locator_id', locator::find_all_of_subinventory($$class_second->to_subinventory_id), 'locator_id', 'locator', $$class_second->to_locator_id, '', $readonly, 'subinventory_id'); ?>
         </td>
        </tr>
       </tbody>
      </table>
      ...
      <table class="form_line_data_table">
       ...
       <tbody class="inv_transaction_values form_data_line_tbody">
        <tr class="wip_wol_transaction0" id="tab4_1">
         <td><?php $f->ac_field_wid2('account_id'); ?></td>
         <td><?php $f->text_field_wid2('unit_cost'); ?></td>
         <td><?php $f->text_field_wid2('costed_amount'); ?></td>
         <td><?php  $f->text_field_wid2('gl_journal_header_id'); ?></td>
        </tr>
       </tbody>
      </table>
     </div>
       ...
       <tbody class="form_data_line_tbody">
        <tr class="wip_wol_transaction0" id="tab4_1">
         <td class="add_detail_values0">	<?php
          echo!empty($$class->lot_number_id) ? $f->hidden_field('lot_number_id', $$class_second->lot_number_id) : $f->hidden_field('lot_number_id', '');
          echo!empty($$class->lot_generation) ? $f->hidden_field('lot_generation', $$class_second->lot_generation) : $f->hidden_field('lot_generation', '');
          ?> 
                  ...
                  <tr class="inv_serial_number<?php echo $detailCount; ?><?php echo $detailCount != 0 ? ' new_object' : '' ?>">
                   <td>   
                    <ul class="inline_action">
                     ...
                     <li><?php echo form::hidden_field('inv_transaction_id', $$class_second->inv_transaction_id); ?></li>
                    </ul>
                   </td>
                   <td><?php
