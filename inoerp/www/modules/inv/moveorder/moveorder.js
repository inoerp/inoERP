...
//Popup for adding moveorder lines
function add_moveorder_lines() {
 var inv_moveorder_header_id = $("#inv_moveorder_header_id").val();
 if (inv_moveorder_header_id) {
  var link = 'multi_select.php?class_name=po_all_v&action=multi_moveorder&mode=9&action_class_name=inv_moveorder_line&';
  link += 'po_status=APPROVED&inv_moveorder_header_id=' + inv_moveorder_header_id + '&org_id=' + $('#org_id').val();
  localStorage.removeItem("reset_link");
  localStorage.setItem("reset_link", link);
  localStorage.removeItem("jsfile");
  localStorage.setItem("jsfile", "modules/inv/moveorder/extra/extra_inv_moveorder.js");
  void window.open(link, '_blank',
          'width=1000,height=800,TOOLBAR=no,MENUBAR=no,SCROLLBARS=yes,RESIZABLE=yes,LOCATION=no,DIRECTORIES=no,STATUS=no');
  return false;
 } else {
  alert('No moveorder Header ID/nEnter or Save The Header Details ');
 }
}
...
//Default header values to line
 $('body').off('change', '.transaction_quantity').on('change', '.transaction_quantity', function () {
  ...
  $(this).closest('.tabContainer').find(trClass).find('.inv_moveorder_header_id').val($('#inv_moveorder_header_id').val());
 });
...
//get Subinventory Name
 ...
 //selecting Header Id
 $(".inv_moveorder_header_id.select_popup").on("click", function () {
  void window.open('select.php?class_name=inv_moveorder_header', '_blank',
          'width=1000,height=800,TOOLBAR=no,MENUBAR=no,SCROLLBARS=yes,RESIZABLE=yes,LOCATION=no,DIRECTORIES=no,STATUS=no');
 });
 //Get the moveorder_id on find button click
// $('a.show.inv_moveorder_header').click(function() {
//  var moveorderId = $('#inv_moveorder_header_id').val();
//  $(this).attr('href', modepath() + 'inv_moveorder_header_id=' + moveorderId);
// });
